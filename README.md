...
| `GET` | `/api/orders` | Получить список заказов, сначала новые |
| `GET` | `/api/orders/{id}` | Получить карточку заказа |

Номер заказа создаётся сервером в формате `ORD-ГГГГММДД-XXXXXXXX`. Все поля обязательны; вес должен быть положительным.
